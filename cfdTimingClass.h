...
//   }
  ...
  // if (max_position > 20 && max_position < 100 && phase[m] > 20 && phase[m]<60){ 
   if (max_position > 20 && max_position < 150 && phase[m] > 20 && phase[m]<260){ 
...
// if (size[chan]-cfdposR<50){
 for (int i = cfdposR-5; i < cfdposR+45; i++){
  if(((double)dTrace->at(i)+(double)dTrace->at(i+1))/2.0>baseline){
   ...
   if (i<cfdposR+4) leadqdc[chan] += ((double)(dTrace->at(i)+dTrace->at(i+1))-baseline*2.0)*0.5;
   }
  else continue;
  }
// }
